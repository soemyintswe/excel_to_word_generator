...
class App:
    ...
    # ---------------------------------------------------------
    # --- Document Generator Logic (100% In-Place Edit) ---
    # ---------------------------------------------------------
    def generate_report(self):
        ...
        try:
            ...
            # မူလ Template ၏ ဇယားသည် Data ဘယ်နှစ်ခု ဆံ့သလဲ တွက်ချက်ခြင်း
            temp_doc = Document(self.template_path)
            table_template = temp_doc.tables[0]
            max_data_rows = len(table_template.rows) - 1 # Header အကွက်ကို ဖယ်ထုတ်သည်
            ...
            # Excel Data အားလုံးကို ဇယားဆံ့သလောက် Batch များခွဲခြင်း
            ...
            for batch_idx in range(total_batches):
                ...
                # မူလ Template ကိုသာ တိုက်ရိုက်ခေါ်သုံးသည် (ဖိုင်အသစ်လုံးဝမဆောက်ပါ)
                doc = Document(self.template_path)
                table = doc.tables[0]
                
                for i, (_, row) in enumerate(batch_df.iterrows()):
                    row_idx_in_word = i + 1 # ဇယား၏ Row 1 (ဒုတိယအကြောင်း) မှစဖြည့်မည်
                    
                    for col_idx, cell in enumerate(table.rows[0].cells):
                        header = cell.text.strip()
                        # Map လုပ်ထားသော Column ဖြစ်မှသာ ဝင်ရောက်ရေးသားမည်
                        # (Map မလုပ်ထားသော "စဉ်" များကို လုံးဝမထိပါ)
                        if header in self.mapping_rules:
                            excel_cols = self.mapping_rules[header]
                            vals = [str(row[c]) for c in excel_cols if pd.notna(row[c])]
                            final_val = " ".join(vals)
                            
                            # ဆဲလ်ထဲသို့ စာသားတိုက်ရိုက်အစားထိုးခြင်း
                            table.rows[row_idx_in_word].cells[col_idx].text = final_val

                # Format လုံးဝမပျက်စေရန်အတွက် Data များလျှင် ဖိုင်ခွဲ၍ Save မည်
                output_filename = f"output/Report_Page_{batch_idx + 1}.docx"
                doc.save(output_filename)

            messagebox.showinfo("Success", f"Format မူလအတိုင်း အတိအကျဖြင့် ဖိုင် ({total_batches}) ဖိုင် အောင်မြင်စွာ ထွက်ရှိပါပြီ။\n'OPEN FOLDER' ကို နှိပ်၍ ဖွင့်ကြည့်နိုင်ပါသည်။")
